...
- **React Project Setup**: Generate a new React project:
  ...
  - **React Packages**: Install the required React packages:
  ...
  - **Development Server**: Start the React development server:
...
- **.env File**: Begin by creating an `.env` file at the root of your Django project to securely store your Openai API key:
...
### Configure Django for Websockets

- **Modify settings.py**: In `settings.py`, add `langchain_stream` and `daphne` to `INSTALLED_APPS`. 
  ```python
  'daphne',
  # ...,
  'langchain_stream',
...
- **Install Packages**: Install LangChain and related packages for LLM integration and websocket support:
In the terminal, run:
...
- **Routing Configuration**: Define how websocket connections are handled by creating `routing.py` and `urls.py` in your `langchain_stream` app.
